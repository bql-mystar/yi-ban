    ...
    def run(self):
        for i in range(30):
            ...
            for li in lis:
                ...
                voptions_id_list, mount_id, author_id = self.get_voptions_id(params_dict)
                # 发表评论
                post_response = self.add_comment(params_dict, mount_id, author_id)
                dict_response = json.loads(post_response)
                print(dict_response)
                if dict_response.get('code') == 200:
                    print('评论成功')
            # 已经评论完一页了，让代码休息十分钟不过分吧
            time.sleep(5)
